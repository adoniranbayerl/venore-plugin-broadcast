...
export async function applyOutputTicker(input: { id: string; tickerEnabled: boolean }): Promise<BroadcastOutputRecord> {
  const [row] = await db
    .update(broadcastOutputs)
    .set({ tickerEnabled: input.tickerEnabled, updatedAt: sql`now()` })
