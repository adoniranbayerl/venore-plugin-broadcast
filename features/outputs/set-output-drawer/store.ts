...
export async function applyOutputDrawer(input: { id: string; drawerOpen: boolean }): Promise<BroadcastOutputRecord> {
  const [row] = await db
    .update(broadcastOutputs)
    .set({ drawerOpen: input.drawerOpen, updatedAt: sql`now()` })
