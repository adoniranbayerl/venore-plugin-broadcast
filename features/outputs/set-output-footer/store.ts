...
export async function applyOutputFooter(input: { id: string; footerOpen: boolean }): Promise<BroadcastOutputRecord> {
  const [row] = await db
    .update(broadcastOutputs)
    .set({ footerOpen: input.footerOpen, updatedAt: sql`now()` })
